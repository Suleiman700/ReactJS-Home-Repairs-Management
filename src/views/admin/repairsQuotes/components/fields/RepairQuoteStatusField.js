...
const RepairQuoteStatusField = ({ value, onChange, isRequired, options }) => {
    ...
    return (
        <FormControl>
            ...
            >
                {options.map((option) => (
                    <option key={option.key} value={option.key} selected={option.key==value?true:false}>
                        {option.name}
                    </option>
                ))}
            </Select>
...
export default RepairQuoteStatusField;
